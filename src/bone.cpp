...
inline uint32_t getIndex(const float animationTime, const KeyScale* scales){
	...
	if(prevResult < numScalings || animationTime > scales[prevResult].timeStamp)
		#if BINARYSEARCHINDEX
			prevResult = binarySearchIndex(animationTime, scales, 0, numScalings) - 1;
		#else
			for (uint32_t i = 0; i != (numScalings - 1); i++){
...
inline const glm::mat4 interpolate(const float animationTime, const KeyPosition* positions){
	...
	return glm::translate(glm::mat4(1.0f),mix(positions[index].position, positions[index + 1].position, 1.f));
}
...
inline const glm::mat4 interpolate(const float animationTime, const KeyRotation* rotations){
	...
	float fact = GetScaleFactor(rotations[index].timeStamp, rotations[index + 1].timeStamp, animationTime);
	...
	*/
	...
	if(G_squad){
		N_returned = glm::slerp(glm::normalize(rotations[index].orientation), glm::normalize(rotations[index+1].orientation), 1.f);
		...
		/*O_returned = glm::squad(m_Rotations[index].orientation, m_Rotations[index+1].orientation,
				...
				scalarFactor);*/
		O_returned = all_in_one_squad(rotations[index-1].orientation, rotations[index].orientation, rotations[index+1].orientation, rotations[index+2].orientation, fact);
...
inline const glm::mat4 interpolate(float animationTime, const KeyScale* scales){
	...
	return glm::scale(glm::mat4(1.0f), mix(scales[index].scale, scales[index+1].scale, 1.f));
}
